...
class CryptoDataService:
    ...
    def upsert_project(
        self, coin_data: Dict, data_source: str = "livecoinwatch"
    ) -> CryptoProject:
        ...
        with self.get_session() as session:
            ...
            logger.success(
                f"Processed project: {project.name} ({len(changes)} changes tracked)"
            )

            return project

    def _track_project_changes(
        self, project: CryptoProject, new_data: Dict, data_source: str
    ) -> List[ProjectChange]:
        ...
        for field_name, new_value in basic_fields.items():
            ...
            if self.change_tracker.has_changed(old_value, new_value):
                changes.append(
                    self.change_tracker.create_change_record(
    ...
    def _process_project_links(
        self, session: Session, project: CryptoProject, links_data: Dict
    ):
    ...
    def _process_project_images(
        self, session: Session, project: CryptoProject, coin_data: Dict
    ):
    ...
    def get_project_changes(
        self, project_id: int, field_name: str = None, limit: int = 100
    ) -> List[ProjectChange]:
    ...
    def get_recent_changes(
        ...
        with self.get_session() as session:
            ...
            return (
                ...
                .filter(ProjectChange.created_at >= cutoff_time)
                .order_by(ProjectChange.created_at.desc())
                .limit(limit)
                .all()
            )

    def get_projects_needing_link_analysis(
        self, limit: int = 50
    ) -> List[Tuple[CryptoProject, List[ProjectLink]]]:
        """Get projects with links that need LLM analysis."""

        with self.get_session() as session:
            # Get projects that have links needing analysis
            projects_with_pending_links = (
                session.query(CryptoProject)
                .join(ProjectLink)
                .filter(ProjectLink.needs_analysis == True)
                .distinct()
                .limit(limit)
                .all()
            )
            ...
            for project in projects_with_pending_links:
                pending_links = (
                    session.query(ProjectLink)
                    .filter_by(project_id=project.id, needs_analysis=True)
                    .all()
                )
    ...
    def get_project_stats(self) -> Dict[str, int]:
        ...
        with self.get_session() as session:
            ...
            pending_analysis = (
                session.query(ProjectLink).filter_by(needs_analysis=True).count()
            )
            ...
            recent_changes = (
                session.query(ProjectChange)
                .filter(ProjectChange.created_at >= recent_cutoff)
                .count()
            )
    ...
    def cleanup_old_changes(self, days_to_keep: int = 90) -> int:
        ...
        with self.get_session() as session:
            ...
            deleted_count = (
                session.query(ProjectChange)
                .filter(ProjectChange.created_at < cutoff_date)
                .delete()
            )
