...
class CrawlJob(Base):
    ...
    # Link to project
    link_id = Column(Integer, ForeignKey("project_links.id"), nullable=False, index=True)
    project_id = Column(Integer, ForeignKey("crypto_projects.id"), nullable=False, index=True)
    ...
    crawler_engine = Column(String(20), default="brozzler")  # brozzler, heritrix, simple
    ...
    # Relationships
    warc_files = relationship("WARCFile", back_populates="crawl_job", cascade="all, delete-orphan")
    snapshots = relationship("WebsiteSnapshot", back_populates="crawl_job", cascade="all, delete-orphan")
...
class WARCFile(Base):
    ...
    cdx_records = relationship("CDXRecord", back_populates="warc_file", cascade="all, delete-orphan")
...
class WebsiteSnapshot(Base):
    ...
    id = Column(Integer, primary_key=True)
    snapshot_uuid = Column(UUID(as_uuid=True), default=uuid.uuid4, unique=True, index=True)
    ...
    version_number = Column(Integer, nullable=False)  # Sequential version for this project
...
class SnapshotChangeDetection(Base):
    ...
    # Snapshot references
    old_snapshot_id = Column(Integer, ForeignKey("website_snapshots.id"), nullable=False, index=True)
    new_snapshot_id = Column(Integer, ForeignKey("website_snapshots.id"), nullable=False, index=True)
    ...
    requires_reanalysis = Column(Boolean, default=False)  # Should trigger LLM reanalysis
    ...
    # Relationships
    old_snapshot = relationship("WebsiteSnapshot", foreign_keys=[old_snapshot_id], back_populates="changes_as_old")
    new_snapshot = relationship("WebsiteSnapshot", foreign_keys=[new_snapshot_id], back_populates="changes_as_new")
    
    __table_args__ = (
        Index('idx_change_detection_snapshots', 'old_snapshot_id', 'new_snapshot_id'),
        Index('idx_change_detection_significant', 'is_significant_change', 'change_score'),
        {'extend_existing': True}
    )
...
class CDXRecord(Base):
    ...
    # Association
    warc_file_id = Column(Integer, ForeignKey("warc_files.id"), nullable=False, index=True)
    snapshot_id = Column(Integer, ForeignKey("website_snapshots.id"), nullable=False, index=True)
...
class CrawlSchedule(Base):
    ...
    # Association
    link_id = Column(Integer, ForeignKey("project_links.id"), nullable=False, unique=True)
