...
class WebsiteAnalysisPipeline:
    ...
    def discover_projects_for_analysis(self, limit: int = None) -> List[Tuple[CryptoProject, ProjectLink]]:
        """
    ...
    def scrape_and_analyze_website(self, project: CryptoProject, website_link: ProjectLink) -> Optional[LinkContentAnalysis]:
        """
        ...
        """
        logger.info(f"Starting analysis for {project.name} ({project.code}) - {website_link.url}")
        ...
        try:
            ...
            if not scrape_result.scrape_success:
                logger.error(f"Scraping failed for {website_link.url}: {scrape_result.error_message}")
            ...
            logger.success(f"Analysis complete for {project.name}: Technical depth {website_analysis.technical_depth_score}/10")
    ...
    def _update_scrape_status(self, website_link: ProjectLink, success: bool, error: str = None):
