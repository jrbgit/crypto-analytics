...
try:
    ...
    logger.warning(
        "Google API Client not installed. Run: pip install google-api-python-client google-auth google-auth-oauthlib google-auth-httplib2"
    )
...
class YouTubeVideo:
    ...
    category_id: str
    video_type: (
        str  # 'educational', 'announcement', 'ama', 'technical', 'marketing', 'other'
    )
    content_hash: str
...
class YouTubeScraper:
    ...
    def __init__(
        self, recent_days: int = 90, max_videos: int = 50, rate_limit_delay: float = 0.1
    ):
        """
        ...
        if not YOUTUBE_API_AVAILABLE:
            logger.warning(
                "YouTube API client not available - install google-api-python-client"
            )
            return
        ...
        try:
            ...
            if (
                ...
                or client_id == "your_youtube_client_id_here"
            ):
                logger.warning(
                    "YouTube OAuth credentials not properly configured - YouTube scraping disabled"
                )
                logger.info(
                    "💡 To enable YouTube analysis, set YOUTUBE_CLIENT_ID and YOUTUBE_CLIENT_SECRET in config/env"
                )
                logger.info(
                    "   Get OAuth credentials at: https://console.cloud.google.com/apis/credentials"
                )
                return
            ...
            logger.info(
                "🔗 YouTube API connection initialized successfully with OAuth 2.0"
            )
            ...
            if e.resp.status == 403:
                ...
                logger.info(
                    "   Check your API quota at: https://console.cloud.google.com/apis/api/youtube.googleapis.com/quotas"
                )
            elif e.resp.status == 400:
                logger.error(
                    "❌ YouTube API request invalid - check API key configuration"
                )
            else:
        ...
        if not self.youtube_available:
            logger.info(
                "📴 YouTube analysis disabled until API key is configured properly"
            )
        ...
        if not credentials or not credentials.valid:
            ...
            if not credentials:
                ...
                try:
                    ...
                    print(f"{auth_url}\n")
                    print(
                        "After authorizing, you'll be redirected to a page that shows 'Bad Gateway'."
                    )
                    print(
                        "Copy the full URL from your browser address bar and paste it here."
                    )
                    print(
                        "The URL should look like: http://localhost:8080/?state=...&code=..."
                    )
                    ...
                    logger.info(
                        "💡 Make sure you copy the authorization code correctly"
                    )
                    return None
        ...
        try:
            ...
            if "/channel/" in parsed.path:
                ...
                if len(path_parts) >= 1 and path_parts[0]:
                    ...
                    logger.debug(
                        f"Attempting to resolve custom handle: {custom_handle}"
                    )
    ...
    def classify_video_type(self, title: str, description: str, tags: List[str]) -> str:
        ...
        educational_count = sum(
            1 for keyword in self.educational_keywords if keyword in combined_text
        )
    ...
    def get_channel_info(self, channel_id: str) -> Optional[YouTubeChannelInfo]:
        ...
        try:
            ...
            profile_image_url = thumbnails.get("high", {}).get(
                "url", ""
            ...
            if e.resp.status == 404:
                ...
                logger.error(
                    f"Access denied to YouTube channel {channel_id} - API quota or permissions issue"
                )
            else:
                logger.error(
                    f"YouTube API error getting channel info for {channel_id}: {e}"
                )
            return None
        except Exception as e:
    ...
    def get_channel_videos(self, channel_id: str) -> List[YouTubeVideo]:
        ...
        if not self.youtube_available:
            logger.error(
                f"YouTube API not available for scraping channel: {channel_id}"
            )
            return []
        ...
        try:
            ...
            while videos_fetched < self.max_videos:
                ...
                for video_data in video_response["items"]:
                    try:
                        ...
                        # Get thumbnail URL
                        thumbnails = snippet.get("thumbnails", {})
                        thumbnail_url = thumbnails.get("high", {}).get(
                            "url", ""
                        ) or thumbnails.get("default", {}).get("url", "")

                        # Classify video type
                        video_type = self.classify_video_type(
                            snippet["title"], snippet.get("description", ""), tags
                        )
            ...
            if videos:
                logger.success(
                    f"Successfully fetched {len(videos)} videos from channel {channel_id}"
                )
            else:
                logger.info(
                    f"No recent videos found in channel {channel_id} (within last {self.recent_days} days)"
                )
            ...
            logger.error(
                f"Unexpected error fetching videos from channel {channel_id}: {e}"
            )
            return []

    def calculate_channel_metrics(
        self, videos: List[YouTubeVideo], channel_info: Optional[YouTubeChannelInfo]
    ) -> Dict:
        ...
        if len(sorted_videos) > 1:
            date_range = (
                sorted_videos[0].published_at - sorted_videos[-1].published_at
            ).days
            upload_frequency_score = (
                len(sorted_videos) / max(date_range / 7.0, 1)
                if date_range > 0
                else len(sorted_videos)
            )
        else:
        ...
        if len(sorted_videos) > 2:
            ...
            for i in range(len(sorted_videos) - 1):
                interval = (
                    sorted_videos[i].published_at - sorted_videos[i + 1].published_at
                ).days
                upload_intervals.append(interval)

            # Lower variance = more consistent
            avg_interval = sum(upload_intervals) / len(upload_intervals)
            variance = sum((x - avg_interval) ** 2 for x in upload_intervals) / len(
                upload_intervals
            )
            consistency_score = max(
                0, 10 - (variance / avg_interval if avg_interval > 0 else 10)
            )
        else:
        ...
        content_type_distribution = {}
        for video in videos:
            content_type_distribution[video.video_type] = (
                content_type_distribution.get(video.video_type, 0) + 1
            )

        # Educational content ratio
        educational_count = content_type_distribution.get(
            "educational", 0
        ) + content_type_distribution.get("technical", 0)
        ...
        if channel_info and channel_info.subscriber_count > 0:
            ...
            sub_to_video_ratio = channel_info.subscriber_count / max(
    ...
    def scrape_youtube_channel(self, youtube_url: str) -> YouTubeAnalysisResult:
        ...
        try:
            ...
            if not videos:
                ...
                if not channel_info:
                    error_message = (
                        "Channel appears to be inaccessible or has no content"
                    )
