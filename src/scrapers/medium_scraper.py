...
class MediumScraper:
    ...
    def __init__(self, max_articles: int = 20, recent_days: int = 90, delay: float = 5.0):
        ...
        self.technical_keywords = [
            'technical', 'development', 'code', 'blockchain', 'protocol', 'smart contract',
            'architecture', 'implementation', 'upgrade', 'fork', 'consensus', 'node',
            'testnet', 'mainnet', 'sdk', 'api', 'integration', 'security', 'audit'
        ]
        
        self.announcement_keywords = [
            'announcement', 'launched', 'release', 'introducing', 'partnership',
            'collaboration', 'funding', 'investment', 'listing', 'exchange'
        ]
        
        self.marketing_keywords = [
            'community', 'event', 'meetup', 'conference', 'ama', 'giveaway',
            'competition', 'reward', 'airdrop', 'token', 'brand'
        ]
    ...
    def classify_article_type(self, title: str, content: str, tags: List[str]) -> str:
        ...
        """
        text_to_analyze = f"{title} {content[:1000]}".lower()  # First 1000 chars of content
        ...
        update_count = sum(1 for keyword in self.update_keywords if keyword in combined_text)
    ...
    def parse_feed(self, feed_url: str) -> List[MediumArticle]:
        """
        Parse Medium RSS feed and extract article information.
    ...
    def calculate_publication_metrics(self, articles: List[MediumArticle]) -> Dict:
        ...
        if len(sorted_articles) > 1:
            date_range = (sorted_articles[0].published_date - sorted_articles[-1].published_date).days
            if date_range > 0:
        ...
        for article in articles:
            content_distribution[article.article_type] = content_distribution.get(article.article_type, 0) + 1
        
        # Average reading time
        reading_times = [a.reading_time for a in articles if a.reading_time > 0]
        avg_reading_time = sum(reading_times) / len(reading_times) if reading_times else 0.0
    ...
    def scrape_medium_publication(self, medium_url: str) -> MediumAnalysisResult:
        ...
        try:
            ...
            logger.success(f"Medium scraping complete: {len(articles)} articles analyzed")
            return result
...
if __name__ == "__main__":
    ...
    for url in test_urls:
        ...
        if result.scrape_success:
            ...
            print(f"Publication frequency: {result.publication_frequency:.2f} posts/week")
