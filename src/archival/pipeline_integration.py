...
class ArchivalPipelineIntegration:
    ...
    def on_project_discovered(
        ...
        try:
            ...
            logger.info(f"Created initial crawl job {job.job_id} for project {project_id}")
        ...
        with self.db.session() as session:
            ...
            significant_changes = session.execute(
                ...
                .filter(
                    ...
                    SnapshotChangeDetection.overall_change_score >= reanalysis_threshold
                )
                .order_by(SnapshotChangeDetection.comparison_timestamp.desc())
            ).scalars().all()
            
            logger.info(f"Found {len(significant_changes)} changes requiring reanalysis")
            ...
            for change in significant_changes:
                ...
                if change.llm_reanalysis_triggered_at:
                    logger.debug(f"Reanalysis already triggered for project {project_id}")
                    continue
    ...
    def trigger_reanalysis_for_project(
        self,
        project_id: int,
        reason: str = "Significant website change detected"
    ) -> bool:
        """
    ...
    def get_archival_status_for_project(self, project_id: int) -> dict:
        ...
        with self.db.session() as session:
            ...
            return {
                ...
                'latest_snapshot': {
                    'timestamp': latest_snapshot.snapshot_timestamp.isoformat() if latest_snapshot else None,
                    'version': latest_snapshot.version_number if latest_snapshot else None,
                    'pages': latest_snapshot.total_pages_archived if latest_snapshot else None
                ...
                'schedule_frequency': str(schedule.crawl_frequency) if schedule else None
            }
    ...
    def update_pipeline_status_with_archival(
        ...
        if status['has_schedule']:
            status_logger.log_info(
                f"Automated crawls: {status['schedule_frequency']}"
            )
