...
class WARCStorageManager:
    ...
    def _init_azure_client(self):
        ...
        try:
            ...
            logger.info(
                f"Azure client initialized for container: {self.config.azure_container}"
            )
        except ImportError:
            logger.error(
    ...
    def generate_warc_filename(
        self, project_code: str, timestamp: datetime, sequence: int = 1
    ) -> str:
        """
    ...
    def get_storage_path(self, filename: str, timestamp: datetime) -> Path:
        """
    ...
    def create_warc_writer(
        self, file_path: Path, warc_version: str = "1.1"
    ) -> WARCWriter:
        """
        ...
        warcinfo_payload = (
            b"software: Crypto Analytics Archival System\n"
    ...
    def store_warc_file(
        ...
        if self.config.backend == "s3":
            ...
            logger.info(
                f"Uploaded WARC to S3: s3://{self.config.s3_bucket}/{remote_key}"
            )
            ...
            blob_client = self.azure_client.get_blob_client(
                container=self.config.azure_container, blob=remote_key
            )
            ...
            logger.info(
                f"Uploaded WARC to Azure: {self.config.azure_container}/{remote_key}"
            )
    ...
    def retrieve_warc_file(
        self, remote_key: str, local_path: Optional[Path] = None
    ) -> Path:
        """
        ...
        if self.config.backend == "s3":
            self.s3_client.download_file(
                self.config.s3_bucket, remote_key, str(local_path)
            )
    ...
    def list_warc_files(
        ...
        if self.config.backend == "local":
            ...
            response = self.s3_client.list_objects_v2(
                Bucket=self.config.s3_bucket, Prefix=prefix or ""
            )
        ...
        if start_date or end_date:
            files = [
                f
                for f in files
                if (not start_date or f["modified"] >= start_date)
                and (not end_date or f["modified"] <= end_date)
            ]
        ...
        try:
            if self.config.backend == "local":
                ...
                self.s3_client.delete_object(
                    Bucket=self.config.s3_bucket, Key=remote_key
                )
