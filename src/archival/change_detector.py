...
class ChangeDetector:
    ...
    def __init__(
        ...
        reanalysis_threshold: float = 0.3
    ):
        """
    ...
    def detect_changes(
        ...
        """
        logger.info(f"Detecting changes between snapshots {old_snapshot.get('id')} and {new_snapshot.get('id')}")
        ...
        content_changes = self._compare_content(
            old_snapshot.get('content', ''),
            new_snapshot.get('content', '')
        )
        ...
        structure_changes = self._compare_structure(
            old_snapshot.get('html', ''),
            new_snapshot.get('html', '')
        )
        ...
        resource_changes = self._compare_resources(
            old_snapshot.get('resources', []),
            new_snapshot.get('resources', [])
        )
        ...
        page_changes = self._compare_pages(
            old_snapshot.get('urls', []),
            new_snapshot.get('urls', [])
        )
        ...
        # Set flags
        metrics.is_significant_change = metrics.change_score >= self.significance_threshold
    ...
    def _hashes_match(self, old_snapshot: Dict, new_snapshot: Dict) -> bool:
        ...
        old_hash = old_snapshot.get('content_hash_sha256') or old_snapshot.get('full_site_hash_sha256')
        new_hash = new_snapshot.get('content_hash_sha256') or new_snapshot.get('full_site_hash_sha256')
    ...
    def _compare_structure(self, old_html: str, new_html: str) -> Dict:
        ...
        try:
            ...
            if old_sections and new_sections:
                ...
                diff_score = sections_changed / total_sections if total_sections > 0 else 0.0
            else:
                diff_score = 1.0 if old_html != new_html else 0.0
    ...
    def _calculate_change_score(self, metrics: ChangeMetrics) -> float:
        ...
        resources_score = min(1.0, total_resources / 50.0)  # Normalize to 50 resource changes = 1.0
        
        # Pages score
        total_page_changes = len(metrics.pages_added or []) + len(metrics.pages_removed or [])
        pages_score = min(1.0, total_page_changes / 20.0)  # Normalize to 20 page changes = 1.0
    ...
    def compute_structure_hash(self, html: str) -> str:
        ...
        try:
            ...
            for element in soup.find_all():
                structure_elements.append(f"{element.name}:{element.get('id', '')}:{'.'.join(element.get('class', []))}")
...
def format_change_report(metrics: ChangeMetrics) -> str:
    ...
    report.append(f"Requires Reanalysis: {'Yes' if metrics.requires_reanalysis else 'No'}")
    report.append("")
