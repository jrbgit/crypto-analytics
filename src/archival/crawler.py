...
class CrawlConfig:
    ...
    # User agent
    user_agent: str = "Mozilla/5.0 (compatible; CryptoAnalytics/1.0; +http://cryptoanalytics.io/bot)"
...
class ArchivalCrawler:
    ...
    def crawl(
        ...
        """
        logger.info(f"Starting crawl of {config.seed_url} with engine: {config.crawler_engine}")
        ...
        try:
            ...
            logger.success(f"Crawl completed: {result.pages_crawled} pages in {result.crawl_duration:.1f}s")
    ...
    def _crawl_with_browsertrix(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            crawl_config = {
                ...
                "saveState": "never",
                "behaviors": "autoscroll,autoplay,autofetch,siteSpecific" if config.use_javascript_rendering else "",
                "behaviorTimeout": config.javascript_timeout * 1000,  # Convert to milliseconds
            ...
            docker_cmd = [
                "docker", "run", "--rm",
                "-v", f"{temp_path}:/output",
                "-v", f"{config_file}:/app/crawl-config.json:ro",
                "webrecorder/browsertrix-crawler",
                "crawl", "--config", "/app/crawl-config.json"
            ]
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(f"Crawl timeout after {config.timeout_seconds}s, killing container...")
                    process.kill()
                    try:
                        # Also try to find and stop the container
                        subprocess.run(
                            ["docker", "ps", "-q", "--filter", "ancestor=webrecorder/browsertrix-crawler"],
                            capture_output=True,
                            text=True,
                            timeout=10
                        )
                ...
                return CrawlResult(
                    ...
                    bytes_downloaded=final_path.stat().st_size if final_path.exists() else 0,
    ...
    def _crawl_with_brozzler(
        ...
        Returns:
            CrawlResult
        """
        logger.warning("Brozzler integration not fully implemented. Falling back to simple crawler.")
    ...
    def _crawl_simple(
        ...
        logger.info(f"Simple crawl complete: {pages_crawled} pages, {bytes_downloaded} bytes")
    ...
    def extract_warc_metadata(self, warc_path: Path) -> Dict:
        ...
        try:
            ...
            logger.info(f"Extracted metadata from WARC: {metadata['record_count']} records")
