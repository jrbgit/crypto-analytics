...
class ArchivalScheduler:
    ...
    def __init__(
        ...
        jobstores = {
            'default': SQLAlchemyJobStore(url=str(db_manager.engine.url))
        }
        executors = {
            'default': ThreadPoolExecutor(max_concurrent_crawls)
        }
        job_defaults = {
    ...
    def _load_schedules(self) -> None:
        ...
        with get_db_session() as session:
            schedules = session.execute(
                ...
                .filter(CrawlSchedule.enabled == True)
            ).scalars().all()
    ...
    def _create_job(self, schedule: CrawlSchedule) -> None:
        ...
        logger.info(
            f"Scheduled job {job_id} with frequency {schedule.crawl_frequency}"
        )
    ...
    def _execute_crawl(self, schedule_id: int) -> None:
        ...
        with get_db_session() as session:
            ...
            try:
                ...
                logger.error(
                    f"Error executing scheduled crawl {schedule_id}: {e}",
                    exc_info=True
                )
                ...
                if schedule.consecutive_failures >= 5:
                    logger.error(
                        f"Schedule {schedule_id} disabled after 5 failures"
                    )
