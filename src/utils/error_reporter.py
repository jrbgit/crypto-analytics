...
class ErrorReporter:
    ...
    def __init__(self, report_file: Optional[str] = None):
        ...
        """
        self.report_file = Path(report_file) if report_file else Path("error_report.json")
    ...
    def generate_summary(self) -> Dict[str, Any]:
        ...
        severity_order = {ErrorSeverity.CRITICAL: 0, ErrorSeverity.HIGH: 1, 
                         ErrorSeverity.MEDIUM: 2, ErrorSeverity.LOW: 3}
        ...
        recommendations = self._generate_recommendations(summaries, category_counts, severity_counts)
    ...
    def _generate_recommendations(
        ...
        if critical_errors:
            ...
            for error in critical_errors[:3]:  # Top 3 critical
                recommendations.append(f"  • {error.error_type}: {error.suggested_fixes[0] if error.suggested_fixes else 'Investigate immediately'}")
        
        # Network issues
        if category_counts[ErrorCategory.NETWORK.value] > len(self.errors) * 0.3:
            recommendations.append("🌐 High network error rate - check internet connection and target site availability")
        
        # Content issues
        if category_counts[ErrorCategory.CONTENT.value] > len(self.errors) * 0.4:
            recommendations.append("📄 Many content-related issues - consider updating URL lists and checking site status")
        
        # API issues
        if category_counts[ErrorCategory.API.value] > 0:
            recommendations.append("🤖 API-related errors detected - verify LLM service status and configuration")
        
        # Configuration issues
        if category_counts[ErrorCategory.CONFIGURATION.value] > 0:
            recommendations.append("⚙️ Configuration issues found - review setup and dependencies")
        
        # High error rate
        if severity_counts['high'] + severity_counts['critical'] > len(self.errors) * 0.2:
            recommendations.append("⚠️ High severity error rate - consider pausing analysis to fix major issues")
        
        # Success rate recommendations
        success_rate = self._calculate_success_rate()
        if success_rate < 50:
            recommendations.append("📉 Low success rate (<50%) - review error patterns and fix systematic issues")
        elif success_rate < 75:
            recommendations.append("📊 Moderate success rate - focus on fixing most common error types")
        else:
            recommendations.append("✅ Good success rate - continue monitoring and address remaining issues")
    ...
    def _calculate_success_rate(self) -> float:
        ...
        estimated_failures = (critical_errors * 5) + (high_errors * 2) + len(self.errors)
        estimated_attempts = estimated_failures * 2  # Assume 50% base success rate
        
        return max(0, min(100, ((estimated_attempts - estimated_failures) / estimated_attempts) * 100))
    ...
    def _get_problematic_domains(self) -> List[Dict[str, Any]]:
        ...
        for domain, errors in domain_errors.items():
            if len(errors) >= 2:  # Only include domains with multiple errors
                severity_score = sum(
                    ...
                    2 if e.severity == ErrorSeverity.MEDIUM else 1
                    for e in errors
                )
        ...
        return sorted(problematic_domains, key=lambda x: (-x['severity_score'], -x['error_count']))[:10]
    ...
    def print_summary(self, max_errors: int = 10) -> None:
        ...
        for i, error_summary in enumerate(summary['error_summaries'][:max_errors]):
            ...
            print(f"{emoji} {error_summary['error_type']}: {error_summary['count']} occurrences")
        ...
        if summary['most_problematic_domains']:
            ...
            for domain_info in summary['most_problematic_domains'][:5]:
                print(f"• {domain_info['domain']}: {domain_info['error_count']} errors ({', '.join(domain_info['error_types'])})")
...
def generate_error_report(save_to_file: bool = True, print_summary: bool = True) -> Dict[str, Any]:
...
if __name__ == "__main__":
    ...
    error_reporter.log_error("dns_resolution_error", "Failed to resolve domain", "https://nonexistent.example.com")
    error_reporter.log_error("llm_analysis_failed", "Ollama connection timeout", context={"model": "llama3.1:latest"})
    error_reporter.log_error("pdf_extraction_failed", "Corrupted PDF file", "https://example.com/whitepaper.pdf")
