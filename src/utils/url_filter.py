...
class URLFilter:
    ...
    def __init__(self):
        self.skip_patterns_compiled = [
            re.compile(pattern, re.IGNORECASE) for pattern in self.SKIP_PATTERNS
        ]
        self.domain_patterns_compiled = [
            re.compile(pattern, re.IGNORECASE) for pattern in self.PROBLEMATIC_DOMAINS
        ]
    ...
    def _has_templated_content_pattern(self, content_lower: str) -> bool:
        ...
        template_count = sum(
            1 for pattern in template_patterns if pattern in content_lower
        )
