...
class MediumAnalysis:
    ...
    content_breakdown: Dict[
        str, int
    ]  # Count by type: technical, announcement, marketing, update, other
...
"""
            ...
            if self.db_manager:
                try:
                    ...
                    with self.db_manager.get_session() as session:
                        ...
                        session.commit()
                        logger.debug(
                            f"Ollama medium usage: {estimated_tokens} tokens, {response_time:.2f}s"
                        )
                except Exception as e:
        ...
        for i, article in enumerate(scrape_result.articles_found):
            if i < 5:  # Full analysis for first 5
                article_summary = [
                    ...
                    (
                        f"Content preview: {article.content[:1000]}..."
                        if len(article.content) > 1000
                        else f"Content: {article.content}"
                    ),
                ]
    ...
    def analyze_medium_publication(
        self, scrape_result: MediumAnalysisResult
    ) -> Optional[MediumAnalysis]:
        """
        ...
        logger.info(
            f"Starting LLM analysis of {scrape_result.publication_name} with {len(scrape_result.articles_found)} articles"
        )
        ...
        technical_percentage = (
            content_breakdown.get("technical", 0) / total_articles * 100
            if total_articles > 0
            else 0
        )
        marketing_percentage = (
            (
                content_breakdown.get("marketing", 0)
                + content_breakdown.get("announcement", 0)
            )
            / total_articles
            * 100
            if total_articles > 0
        ...
        try:
            analysis = MediumAnalysis(
                ...
                community_engagement_score=analysis_result.get(
                    "community_engagement_score", 5.0
                ),
                content_quality_score=analysis_result.get("content_quality_score", 5),
                development_activity_score=analysis_result.get(
                    "development_activity_score", 5
                ),
                recent_development_mentions=analysis_result.get(
                    "recent_development_mentions", []
                ),
                roadmap_mentions=analysis_result.get("roadmap_mentions", []),
                team_activity_indicators=analysis_result.get(
                    "team_activity_indicators", []
                ),
                partnership_announcements=analysis_result.get(
                ...
                positioning_statements=analysis_result.get(
                ...
                post_frequency_trend=analysis_result.get(
                    "post_frequency_trend", "stable"
                ),
                # Analysis metadata
                confidence_score=analysis_result.get("confidence_score", 0.7),
                analysis_timestamp=datetime.now(UTC),
                model_used=f"{self.provider}:{self.model}",
                articles_with_detailed_analysis=min(
                    5, len(scrape_result.articles_found)
                ),  # We analyze first 5 in detail
            )

            logger.success(
                f"Medium analysis complete for {scrape_result.publication_name}"
            )
            logger.info(
                f"Engagement score: {analysis.community_engagement_score}/10, Development activity: {analysis.development_activity_score}/10"
            )
...
if __name__ == "__main__":
    ...
    test_urls = ["https://medium.com/@binance"]
    ...
    for url in test_urls:
        ...
        if scrape_result.scrape_success:
            ...
            if analysis:
                ...
                print(
                    f"Technical content: {analysis.technical_content_percentage:.1f}%"
                )
                print(
                    f"Recent development mentions: {len(analysis.recent_development_mentions)}"
                )
            else:
