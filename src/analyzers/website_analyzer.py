...
"""
    ...
    def _call_ollama(self, content: str, max_retries: int = 3) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries + 1):
            ...
            try:
                ...
                if attempt > 0:
                    logger.debug(
                        f"Making Ollama API call attempt {attempt + 1}/{max_retries + 1} with model {self.model}"
                    )
                else:
                ...
                if "error" in result:
                    ...
                    if "model not found" in error_msg.lower():
                        logger.error(
                            f"Ollama model '{self.model}' not found. Available models can be checked with 'ollama list'"
                        )
                        ...
                        if attempt < max_retries:
                            ...
                            logger.warning(
                                f"Ollama resource error, retrying in {wait_time}s: {error_msg}"
                            )
                            time.sleep(wait_time)
                            continue
                        else:
                            logger.error(
                                f"Ollama resource error after {max_retries + 1} attempts: {error_msg}"
                            )
                            return None
                    else:
                ...
                if parsed_json:
                    if attempt > 0:
                        logger.info(
                            f"Ollama API call succeeded on attempt {attempt + 1}"
                        )
                    logger.debug(
                        f"Ollama usage: {estimated_tokens} tokens, {response_time:.2f}s response time"
                    )
                    return parsed_json
                else:
                    if attempt < max_retries:
                        logger.warning(
                            f"Failed to parse JSON from Ollama response on attempt {attempt + 1}, retrying..."
                        )
                        time.sleep(2**attempt)  # Exponential backoff
                        continue
                    else:
                        logger.error(
                            f"Failed to parse valid JSON from Ollama after {max_retries + 1} attempts"
                        )
                        return None
                ...
                if self.db_manager:
                    try:
                        ...
                        logger.warning(
                            f"Failed to log Ollama connection error: {log_error}"
                        )

                if "connection refused" in str(e).lower():
                    logger.error(
                        f"Ollama server not running or not accessible. Please ensure Ollama is running on {self.ollama_base_url}"
                    )
                    return None  # Don't retry connection refused
                elif attempt < max_retries:
                    wait_time = (2**attempt) * 1
                    logger.warning(
                        f"Ollama connection error on attempt {attempt + 1}, retrying in {wait_time}s: {e}"
                    )
                    time.sleep(wait_time)
                    continue
                else:
                    logger.error(
                        f"Ollama connection failed after {max_retries + 1} attempts: {e}"
                    )
                    return None
                ...
                if attempt < max_retries:
                    ...
                    logger.warning(
                        f"Ollama API timeout on attempt {attempt + 1}, retrying with longer timeout (+{timeout_increase}s)"
                    )
                    time.sleep(2**attempt)
                    continue
                else:
                    logger.error(
                        f"Ollama API timeout after {max_retries + 1} attempts: {e}"
                    )
                    return None
                ...
                if e.response.status_code == 404:
                    logger.error(
                        f"Ollama API endpoint not found: {self.ollama_base_url}/api/generate"
                    )
                    return None  # Don't retry 404
                elif e.response.status_code == 500 and attempt < max_retries:
                    wait_time = (2**attempt) * 2
                    logger.warning(
                        f"Ollama server error 500 on attempt {attempt + 1}, retrying in {wait_time}s"
                    )
                    time.sleep(wait_time)
                    continue
                else:
                ...
                if attempt < max_retries:
                    wait_time = (2**attempt) * 1
                    logger.warning(
                        f"Unexpected Ollama error on attempt {attempt + 1}, retrying in {wait_time}s: {e}"
                    )
                    time.sleep(wait_time)
                    continue
                else:
                    logger.error(
                        f"Ollama API call failed after {max_retries + 1} attempts: {e}"
                    )
                    return None
    ...
    def _extract_json_from_response(
        self, response_text: str, provider_name: str
        ...
        try:
            ...
            if isinstance(parsed_json, dict):
                ...
                logger.warning(
                    f"{provider_name} returned non-dict JSON: {type(parsed_json)}"
                )
                return None
    ...
    def _create_minimal_content_summary(self, pages: List[Any]) -> str:
        ...
        for page in pages:
            ...
            excerpt = (
                " ".join(content_words[:100])
                if len(content_words) > 100
                else page.content
            )

            summary_parts.append(
                f"=== {page.page_type.upper()}: {title} ===\n{excerpt}"
            )
    ...
    def _combine_page_contents(self, pages: List[Any]) -> str:
        ...
        for page in pages:
            ...
            combined_content.append(
                page.content[:3000]
            )  # Limit per page to control token usage
    ...
    def analyze_website(
        self, scraped_pages: List[Any], domain: str
        ...
        logger.debug(
            f"Combined content: {len(combined_content)} characters, ~{total_word_count} words"
        )
        ...
        if not raw_analysis:
            logger.warning(
                f"Primary LLM analysis failed for {domain}, attempting fallback strategies"
            )
            ...
            if len(combined_content) > 8000:
                ...
                if raw_analysis:
                    logger.success(
                        f"Fallback analysis with reduced content succeeded for {domain}"
                    )
        ...
        try:
            ...
            analysis = WebsiteAnalysis(
                ...
                marketing_vs_tech_ratio=raw_analysis.get(
                    "marketing_vs_tech_ratio", 0.5
                ),
            ...
            logger.success(
                f"Analysis complete for {domain} - Technical depth: {analysis.technical_depth_score}/10, Quality: {analysis.content_quality_score}/10"
            )
    ...
    def batch_analyze_websites(
        self, website_data: List[Dict]
    ) -> List[Optional[WebsiteAnalysis]]:
        """
        ...
        logger.info(
            f"Batch analysis complete: {successful_analyses}/{len(website_data)} successful"
        )
