...
class TelegramAnalysisMetrics:
    ...
    def _calculate_authenticity_score(self, channel_data: Dict) -> float:
        ...
        if description:
            ...
            if any(word in description for word in ['project', 'blockchain', 'development', 'protocol']):
                score += 1.0
            
            # Technical terms
            if any(word in description for word in ['defi', 'smart contract', 'consensus', 'node']):
                score += 0.5
            
            # Red flag terms
            if any(word in description for word in ['pump', 'moon', 'guaranteed', 'profit', '100x']):
                score -= 2.0
            
            # Scam indicators
            if any(word in description for word in ['free money', 'airdrop', 'giveaway']):
                score -= 1.0
    ...
    def _calculate_content_score(self, channel_data: Dict) -> float:
        ...
        if title:
            ...
            if any(word in title for word in ['protocol', 'network', 'blockchain', 'coin', 'token']):
                score += 1.0
        ...
        if description:
            ...
            # Professional language
            if any(word in description for word in ['technology', 'innovative', 'decentralized']):
                score += 0.5
    ...
    def _determine_health_status(self, overall_score: float, channel_data: Dict) -> TelegramHealthStatus:
    ...
    def _identify_red_flags(self, channel_data: Dict) -> List[str]:
        ...
        if description:
            if any(word in description for word in ['guaranteed profit', 'get rich', 'easy money']):
                red_flags.append("Unrealistic profit promises")
            
            if any(word in description for word in ['airdrop', 'free tokens', 'giveaway']):
    ...
    def _identify_positive_indicators(self, channel_data: Dict) -> List[str]:
        ...
        if description:
            ...
            if any(word in description for word in ['blockchain', 'decentralized', 'protocol']):
