...
def connect_postgres():
    return psycopg2.connect(
        host='localhost', port='5432', database='crypto_analytics',
        user='crypto_user', password=os.getenv('DB_PASSWORD', 'crypto_secure_password_2024')
    )
...
def create_id_mapping():
    ...
    try:
        ...
        sqlite_cursor.execute("SELECT id, code FROM crypto_projects WHERE code IS NOT NULL")
        sqlite_projects = {row['code']: row['id'] for row in sqlite_cursor.fetchall()}
        
        postgres_cursor.execute("SELECT id, code FROM crypto_projects WHERE code IS NOT NULL")
    ...
    try:
        ...
        for row in sqlite_cursor:
            ...
            if row_dict['link_id'] in postgres_link_ids:
                # Clean JSON fields
                json_fields = ['technology_stack', 'core_features', 'use_cases', 'target_audience',
                              'team_members', 'founders', 'advisors', 'partnerships', 'investors',
                              'innovations', 'roadmap_items', 'red_flags', 'key_points', 'entities',
                              'categories', 'recent_updates', 'competitors_mentioned']
...
def main():
    ...
    logger.info("Completing migration - creating ID mappings and migrating links/analysis...")
