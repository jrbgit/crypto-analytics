...
def test_ollama_usage_tracking():
    ...
    try:
        ...
        result = analyzer.analyze_website(test_content, "test", len(test_content.split()))
        
        end_time = datetime.now(UTC)
        
        if result:
            logger.success("✅ Ollama API call successful")
            logger.info(f"Analysis completed: Technical depth: {result.technical_depth_score}/10")
        else:
        ...
        with db_manager.get_session() as session:
            ...
            recent_usage = session.query(APIUsage).filter(
                APIUsage.api_provider == 'ollama',
                APIUsage.created_at >= start_time
...
def show_recent_usage_stats():
    ...
    with db_manager.get_session() as session:
        ...
        stats = session.query(
            ...
            APIUsage.created_at >= datetime.now(UTC).replace(hour=0, minute=0, second=0, microsecond=0)
        ).group_by(
            APIUsage.api_provider
        ).all()
        ...
        for stat in stats:
            ...
            logger.info(f"  ⏱️ Avg response time: {stat.avg_response_time:.3f}s" if stat.avg_response_time else "  ⏱️ Avg response time: N/A")
            logger.info(f"  🎯 Total tokens: {stat.total_tokens:,}" if stat.total_tokens else "  🎯 Total tokens: N/A")
            logger.info(f"  💰 Total credits: {stat.total_credits}" if stat.total_credits else "  💰 Total credits: N/A")
        
        # Show recent failed calls
        failed_calls = session.query(APIUsage).filter(
