...
def test_error_classification():
    ...
    for error_msg, expected_retryable, expected_max_retries, description in test_cases:
        ...
        if is_retryable == expected_retryable and max_retries == expected_max_retries:
            ...
            print(
                f"  Error: '{error_msg}' -> Retryable: {is_retryable}, Max retries: {max_retries}"
            )
            passed += 1
        else:
            ...
            print(
                f"  Expected: Retryable={expected_retryable}, Max={expected_max_retries}"
            )
    ...
    for test_case in test_urls:
        ...
        try:
            ...
            if result.error_type and expected_type.lower() in result.error_type.lower():
                ...
                print(
                    f"✗ FAIL: Expected error type containing '{expected_type}', got '{result.error_type}'"
                )
                failed += 1
...
def test_parked_domain_detection():
    ...
    test_cases = [
        ...
        (
            "Welcome to our cryptocurrency platform. Learn about blockchain technology.",
            False,
            "Real content",
        ),
...
def test_content_quality_assessment():
    ...
    test_cases = [
        ...
        (
            "Welcome to our blockchain platform. We provide decentralized solutions for DeFi.",
            "substantial",
            "Substantial content",
        ),
    ]
    ...
    for content, expected_type, description in test_cases:
        ...
        if content_type == expected_type:
            ...
            print(
                f"  Quality score: {assessment.get('quality_score')}, Word count: {assessment.get('word_count')}"
            )
            passed += 1
        else:
            print(f"✗ FAIL: {description}")
