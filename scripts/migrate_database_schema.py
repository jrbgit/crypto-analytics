...
def run_migration(database_url: str):
    ...
    with engine.connect() as conn:
        ...
        try:
            ...
            conn.execute(
                text("ALTER TABLE crypto_projects ALTER COLUMN code TYPE VARCHAR(100)")
            )
            ...
            for column in numeric_columns:
                ...
                conn.execute(
                    text(
                        f"ALTER TABLE crypto_projects ALTER COLUMN {column} TYPE NUMERIC(30,8)"
                    )
                )
...
def main():
    ...
    logger.info(
        f"Running migration on database: {database_url.split('@')[1] if '@' in database_url else 'local database'}"
    )
    ...
    response = input(
        "Are you sure you want to run this migration? It will modify the database schema. (yes/no): "
    )
    ...
    try:
        ...
        logger.info(
            "  • Supply columns converted to NUMERIC(30,8) for large value support"
        )
        logger.info("  • Price columns converted to NUMERIC(30,8) for better precision")
        logger.info(
            "  • ath_usd column converted to NUMERIC(30,8) for very large values"
        )
