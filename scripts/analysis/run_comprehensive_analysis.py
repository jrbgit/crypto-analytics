...
def validate_and_process_args(args):
    ...
    if args.disable:
        ...
        if invalid_types:
            print(
                f"Error: Invalid analysis types in --disable: {', '.join(sorted(invalid_types))}"
            )
        ...
        if invalid_types:
            print(
                f"Error: Invalid analysis types in --enable: {', '.join(sorted(invalid_types))}"
            )
            print(f"Available types: {', '.join(sorted(available_types))}")
            sys.exit(1)
        ...
        try:
            for pair in args.batch_size.split(","):
                ...
                if content_type not in available_types:
                    print(
                        f"Error: Invalid content type in --batch-size: {content_type}"
                    )
                    print(f"Available types: {', '.join(sorted(available_types))}")
                    sys.exit(1)
                ...
                if size <= 0:
                    print(
                        f"Error: Batch size must be positive, got {size} for {content_type}"
                    )
                    sys.exit(1)
            ...
            print(
                f"Error: Invalid --batch-size format. Expected 'type=number', got: {args.batch_size}"
            )
...
def run_analysis_batch(content_type, pipeline, batch_size, processed_in_session=None):
    ...
    projects = [
        (project, link)
        for project, link in all_projects
    ...
    try:
        ...
        stats = pipeline.run_analysis_batch_with_projects(
            projects_to_process, link_types=[content_type]
        )
    ...
    content_key = (
        f"{content_type}s_analyzed"
        if content_type == "website"
        else f"{content_type}_analyzed"
    )
    ...
    if stats.get("projects_found", 0) > 0:
        success_rate = (
            stats.get("successful_analyses", 0) / stats["projects_found"]
        ) * 100
...
def main():
    ...
    if previous_state:
        ...
        print(
            f"Previous session started: {previous_state.get('started_at', 'unknown')}"
        )
        ...
        if previous_state.get("completed_content_types"):
            print(
                f"Completed content types: {', '.join(previous_state['completed_content_types'])}"
            )
    ...
    for content_type in [
        ...
        if content_type in enabled_types:
            batch_size = custom_batch_sizes.get(
                content_type, default_batch_sizes[content_type]
            )
    ...
    if len(enabled_types) < 5:
        ...
        for content_type, batch_size in batch_configs:
            ...
            if batch_size != default_size:
                print(
                    f"   {content_type}: {batch_size} items (default: {default_size})"
                )
            else:
                print(f"   {content_type}: {batch_size} items")

    # Initialize or restore total stats
    total_stats = analysis_state.get(
        "total_stats",
        {"successful_analyses": 0, "failed_analyses": 0, "projects_found": 0},
    )
    ...
    try:
        ...
        for content_type, batch_size in batch_configs:
            ...
            stats = run_analysis_batch(
                content_type, pipeline, batch_size, processed_in_session
            )

            if stats:
                total_stats["successful_analyses"] += stats.get(
                    "successful_analyses", 0
                )
            ...
            if stats and stats.get("projects_found", 0) > 0:
                wait_time = (
                    300 if content_type == "medium" else 15
                )  # 5 minutes for Medium
                minutes = wait_time // 60
                seconds = wait_time % 60
                if minutes > 0:
                    print(
                        f"\nWaiting {minutes}m {seconds}s before processing next content type..."
                    )
                else:
                    print(
    ...
    if interrupt_requested:
        ...
        print(
            f"🔄 Completed content types: {', '.join(analysis_state.get('completed_content_types', []))}"
        )
    else:
    ...
    if total_stats.get("projects_found", 0) > 0:
        overall_success_rate = (
            total_stats.get("successful_analyses", 0) / total_stats["projects_found"]
        ) * 100
    ...
    try:
        ...
        print(
            f"  • Estimated success rate: {error_summary.get('success_rate', 0):.1f}%"
        )
        if error_summary.get("most_problematic_domains"):
            print(
                f"  • Most problematic domains: {len(error_summary['most_problematic_domains'])}"
            )
    except Exception as e:
