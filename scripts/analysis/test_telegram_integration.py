...
class TelegramIntegrationTester:
    ...
    def _check_prerequisites(self) -> bool:
        ...
        try:
            ...
            if remaining_calls < 3:  # Need at least 3 calls for testing
                logger.error(f"❌ Insufficient API quota: {remaining_calls} calls remaining this minute")
                logger.error("   Need at least 3 API calls to run tests safely")
                return False
            
            logger.success(f"✅ API quota sufficient: {remaining_calls} calls remaining this minute")
        ...
        try:
            ...
            for i, url in enumerate(test_urls):
                ...
                if channel_id != expected_ids[i]:
                    logger.error(f"❌ URL parsing failed: {url} -> {channel_id} (expected {expected_ids[i]})")
                    return False
        ...
        try:
            ...
            for test_channel in test_channels:
                ...
                if not (expected_min <= actual_score <= expected_max):
                    ...
                    logger.error(f"   Expected score: {expected_min}-{expected_max}, Got: {actual_score:.2f}")
                    return False
                
                logger.success(f"✅ {test_channel['name']}: Score {actual_score:.2f} (expected {expected_min}-{expected_max})")
    ...
    def _test_analyzer_integration(self) -> bool:
        ...
        try:
            ...
            if not (0.8 <= quality_score <= 1.0):  # Should be high quality with all fields
        ...
        try:
            ...
            for channel in test_channels:
                ...
                try:
                    ...
                    if expected.get('large_following') and analysis.member_count < expected.get('expected_min_members', 0):
                        logger.warning(f"⚠️ {channel['name']}: Members lower than expected ({analysis.member_count:,})")
                    
                    if expected.get('official') and not analysis.has_username:
                        logger.warning(f"⚠️ {channel['name']}: Expected official channel with username")
                    
                    # Check overall analysis quality
                    if analysis.overall_score < 5.0:
                        logger.warning(f"⚠️ {channel['name']}: Lower score than expected ({analysis.overall_score:.2f})")
                    
                    logger.success(f"✅ {channel['name']}: Analysis complete")
                    logger.info(f"   Score: {analysis.overall_score:.2f}/10, Members: {analysis.member_count:,}")
                    logger.info(f"   Health: {analysis.health_status}, Confidence: {analysis.confidence_score:.2f}")
            ...
            logger.success(f"✅ Real channel tests completed: {successful_tests}/{len(test_channels)} successful")
            
            # Show final API usage
            final_stats = analyzer.get_usage_stats()
            calls_used = initial_stats['minute_usage'] - stats['minute_usage'] if 'initial_stats' in locals() else 0
