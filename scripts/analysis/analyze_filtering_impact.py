...
def analyze_database_urls():
    ...
    for row in rows:
        ...
        status = (
            "completed"
            if scrape_success
            else "pending" if not last_scraped else "failed"
        )
    ...
    print(
        f"   Would be filtered: {len(results['filtered']):,} ({len(results['filtered'])/results['total']*100:.1f}%)"
    )
    print(
        f"   Would be processed: {len(results['allowed']):,} ({len(results['allowed'])/results['total']*100:.1f}%)"
    )
    print()
    ...
    for content_type, stats in sorted(results["by_type"].items()):
        if stats["total"] > 0:
            ...
            print(
                f"   {content_type}: {stats['filtered']:,}/{stats['total']:,} filtered ({pct:.1f}%)"
            )
    print()
    ...
    for status, stats in sorted(results["by_status"].items()):
        if stats["total"] > 0:
            pct = stats["filtered"] / stats["total"] * 100
            print(
                f"   {status}: {stats['filtered']:,}/{stats['total']:,} would be filtered ({pct:.1f}%)"
            )
    print()
