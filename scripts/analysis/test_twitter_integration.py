    ...
    def _check_prerequisites(self) -> bool:
        ...
        try:
            ...
            if remaining_calls < 5:  # Need at least 5 calls for testing
                logger.error(f"❌ Insufficient API quota: {remaining_calls} calls remaining")
                logger.error("   Need at least 5 API calls to run tests safely")
                return False
            
            logger.success(f"✅ API quota sufficient: {remaining_calls} calls remaining")
        ...
        try:
            ...
            for i, url in enumerate(test_urls):
                ...
                if username != expected_usernames[i]:
                    logger.error(f"❌ URL parsing failed: {url} -> {username} (expected {expected_usernames[i]})")
                    return False
    ...
    def _test_analysis_metrics(self) -> bool:
        ...
        try:
            ...
            for test_profile in test_profiles:
                ...
                logger.success(f"✅ {test_profile['name']}: Score {actual_score:.2f} (expected {expected_min}-{expected_max})")
        ...
        try:
            ...
            for account in test_accounts:
                ...
                try:
                    ...
                    if expected.get('large_following') and analysis.followers_count < expected.get('expected_min_followers', 0):
                        logger.warning(f"⚠️ {account['name']}: Followers lower than expected ({analysis.followers_count:,})")
                    
                    if expected.get('verified') and not analysis.verified:
                        logger.warning(f"⚠️ {account['name']}: Expected verified account")
                    
                    if expected.get('old_account') and analysis.account_age_days < 365:
                        logger.warning(f"⚠️ {account['name']}: Account newer than expected ({analysis.account_age_days} days)")
                    
                    # Check overall analysis quality
                    if analysis.overall_score < 5.0:
                        logger.warning(f"⚠️ {account['name']}: Lower score than expected ({analysis.overall_score:.2f})")
                    
                    logger.success(f"✅ {account['name']}: Analysis complete")
                    logger.info(f"   Score: {analysis.overall_score:.2f}/10, Followers: {analysis.followers_count:,}")
            ...
            logger.success(f"✅ Real account tests completed: {successful_tests}/{len(test_accounts)} successful")
