...
def run_mypy_on_file(file_path: str) -> Tuple[int, str]:
    ...
    try:
        ...
        error_lines = [
            line for line in lines if not line.startswith("Success:") and line.strip()
        ]
...
def analyze_type_coverage():
    ...
    if total_files > 0:
        ...
        print(
            f"Files with no type errors: {clean_files}/{total_files} ({coverage:.1f}%)"
        )
...
def suggest_improvements():
    ...
    print(f"\n🛠️  Common fixes:")
    print(
        "- Replace 'datetime.timezone.utc' with 'datetime.timezone.utc' for Python 3.10 compatibility"
    )
