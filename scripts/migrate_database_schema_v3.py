    ...
    with engine.connect() as conn:
        ...
        try:
            ...
            for column in price_columns:
                logger.info(f"Converting {column} to NUMERIC(50,20) for higher precision...")
                conn.execute(text(f"ALTER TABLE crypto_projects ALTER COLUMN {column} TYPE NUMERIC(50,20)"))
...
def main():
    ...
    logger.info(f"Running migration v3 on database: {database_url.split('@')[1] if '@' in database_url else 'local database'}")
    ...
    try:
        ...
        logger.info("  • current_price column now supports 20 decimal places (NUMERIC(50,20))")
        logger.info("  • ath_usd column now supports 20 decimal places (NUMERIC(50,20))")
        logger.info("  • Can accurately store prices down to 0.00000000000000000001 (1e-20)")
