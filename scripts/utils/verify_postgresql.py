...
def verify_postgresql_connection():
    ...
    try:
        ...
        print("+" + "-" * 80 + "+")
        print(
            f"| {'Rank':<4} | {'Code':<8} | {'Name':<20} | {'Price ($)':<12} | {'Market Cap':<15} |"
        )
        print("+" + "-" * 80 + "+")
        ...
        for project in projects:
            ...
            price = (
                f"${project['current_price']:,.2f}"
                if project["current_price"]
                else "N/A"
            )
            market_cap = (
                f"${project['market_cap']:,.0f}" if project["market_cap"] else "N/A"
            )

            print(
                f"| {rank:<4} | {code:<8} | {name:<20} | {price:<12} | {market_cap:<15} |"
            )
        ...
        for usage in api_usage:
            print(
                f"  {usage['api_provider']}: {usage['call_count']} calls, last: {usage['last_call']}"
            )
