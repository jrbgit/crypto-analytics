...
Examples:
  ...
  %(prog)s --dry-run --all                   # Show what would be reset
    ...
    selection.add_argument(
        "--all", action="store_true", help="Reset all failed projects"
    )
    selection.add_argument(
        "--domain",
        type=str,
    ...
    selection.add_argument(
        "--url-contains", type=str, help="Reset projects whose URL contains this string"
    )
...
def format_project_info(
    project: CryptoProject, link: ProjectLink, verbose: bool = False
) -> str:
    ...
    last_scraped = (
        link.last_scraped.strftime("%Y-%m-%d %H:%M") if link.last_scraped else "Never"
    )
    success_status = (
        "✓" if link.scrape_success else "✗" if link.scrape_success is False else "?"
    )
    ...
    if verbose:
        first_fail_str = (
            first_failure.strftime("%Y-%m-%d %H:%M") if first_failure else "None"
        )
        return (
...
def list_failed_projects(args, db_manager):
    ...
    with db_manager.get_session() as session:
        ...
        print()
        print(
            f"{'CODE':<8} {'TYPE':<10} {'S'} {'FAIL':<4} | {'DOMAIN':<25} | LAST SCRAPED"
        )
        print("-" * 80)
    ...
    with db_manager.get_session() as session:
        ...
        if not args.dry_run:
            ...
            if not args.all and len(results) > 10:
                response = input(
                    f"This will reset {len(results)} projects. Continue? (y/N): "
                )
                if response.lower() != "y":
        ...
        for project, link in results:
            print(
                f"{'[DRY RUN] ' if args.dry_run else ''}Resetting: {format_project_info(project, link)}"
            )
...
def main():
    ...
    if not any(
        ...
        if not args.list_failed:
            print(
                "Error: Must specify at least one selection criteria or use --list-failed"
            )
            sys.exit(1)
