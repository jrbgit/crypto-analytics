    ...
    with engine.connect() as conn:
        ...
        try:
            ...
            for column in numeric_columns:
                logger.info(
                    f"Converting {column} to NUMERIC(40,8) for larger value support..."
                )
                conn.execute(
                    text(
                        f"ALTER TABLE crypto_projects ALTER COLUMN {column} TYPE NUMERIC(40,8)"
                    )
                )
...
def main():
    ...
    logger.info(
        f"Running migration v2 on database: {database_url.split('@')[1] if '@' in database_url else 'local database'}"
    )
    ...
    print(
        "This migration will update numeric columns from NUMERIC(30,8) to NUMERIC(40,8)"
    )
    print(
        "This is needed to handle very large cryptocurrency supply values (up to 10^32)"
    )
    ...
    try:
        ...
        logger.info(
            "  • Can now handle values up to ±10^32 (40 digits total, 8 decimal places)"
        )
