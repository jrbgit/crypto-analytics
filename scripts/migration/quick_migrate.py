        ...
        for row in sqlite_cursor:
            if is_valid_project(row):
                ...
                if len(batch) >= batch_size:
                    ...
                    logger.info(f"Migrated: {migrated}, Skipped: {skipped}, Progress: {(migrated/total_valid)*100:.1f}%")
                    batch = []
...
def migrate_other_tables(sqlite_conn, postgres_conn):
    ...
    tables_to_migrate = [
        ('project_links', 'SELECT * FROM project_links WHERE project_id IN (SELECT id FROM crypto_projects)'),
        ('link_content_analysis', 'SELECT * FROM link_content_analysis WHERE link_id IN (SELECT id FROM project_links)'),
        ('api_usage', 'SELECT * FROM api_usage LIMIT 10000'),  # Keep recent API usage only
    ]
    ...
    for table_name, query in tables_to_migrate:
        try:
            ...
            for row in sqlite_cursor:
                ...
                # Clean JSON fields
                json_fields = ['categories', 'technology_stack', 'core_features', 'use_cases']
