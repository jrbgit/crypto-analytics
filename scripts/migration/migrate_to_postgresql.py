...
class DataMigrator:
    ...
    def __init__(self, sqlite_path: str = None, postgres_config: Dict[str, str] = None):
    ...
    def get_table_info(self, sqlite_conn: sqlite3.Connection) -> Dict[str, Dict]:
        ...
        for table in tables:
            ...
            table_info[table] = {"columns": columns, "record_count": count}
    ...
    def transform_row_data(
        self, table_name: str, row_data: Dict
        ...
        for field in json_fields:
            if field in main_data and main_data[field]:
                try:
                    ...
                    logger.warning(
                        f"Invalid JSON in {table_name}.{field}: {main_data[field]}"
                    )
                ...
                if isinstance(main_data[field], str):
                    try:
                        ...
                        dt = datetime.fromisoformat(
                            main_data[field].replace("Z", "+00:00")
                        )
                        main_data[field] = dt
                    except ValueError:
    ...
    def migrate_table(
        ...
        try:
            ...
            logger.info(
                f"Starting migration of {table_name} ({stats.total_records} records)"
            )
            ...
            while offset < stats.total_records:
                ...
                for row in rows:
                    try:
                        ...
                        # Transform data
                        main_data, separate_data = self.transform_row_data(
                            table_name, row_dict
                        )
                        ...
                        # Insert main data
                        main_query, main_values = self.create_insert_query(
                        ...
                        if table_name == "crypto_projects" and batch_failed <= 5:
                            ...
                            logger.debug(
                                f"Skipped project {code[:20]}...: {str(e)[:50]}..."
                            )
                        elif batch_failed <= 5:
                            logger.debug(
                                f"Failed to migrate record from {table_name}: {e}"
                            )
                ...
                logger.info(
                    f"{table_name}: {progress:.1f}% complete "
            ...
            logger.success(
                f"Migration of {table_name} completed: "
    ...
    def verify_migration(
        self, postgres_conn: psycopg2.extensions.connection
    ) -> Dict[str, int]:
        ...
        for table_name in self.migration_stats.keys():
            try:
                ...
                if count == expected:
                    ...
                    logger.warning(
                        f"⚠ {table_name}: {count} records (expected {expected})"
                    )
    ...
    def run_migration(self) -> bool:
        ...
        try:
            ...
            total_records = sum(
                stats.total_records for stats in self.migration_stats.values()
            )
            ...
            logger.info(
                f"Successful tables: {successful_tables}/{len(self.migration_stats)}"
            )

            total_migrated = sum(
                stats.migrated_records for stats in self.migration_stats.values()
            )
            total_failed = sum(
                stats.failed_records for stats in self.migration_stats.values()
            )
            overall_success_rate = (
                (total_migrated / (total_migrated + total_failed)) * 100
            ...
            if overall_success_rate > 95:
                ...
                logger.info(
                    "4. Consider running ANALYZE on tables for optimal performance"
                )
                return True
            else:
...
def main():
    ...
    if success:
        ...
        logger.info(
            "Set DATABASE_URL='postgresql://crypto_user:${DB_PASSWORD}@localhost:5432/crypto_analytics'"
        )
