...
def index_single_warc(db_manager: DatabaseManager, warc_file_id: int) -> bool:
    ...
    with db_manager.get_session() as session:
        ...
        success = indexer.generate_and_store_index(warc_file_id, warc_file.snapshot_id)
...
def index_snapshot_warcs(db_manager: DatabaseManager, snapshot_id: int) -> bool:
    ...
    with db_manager.get_session() as session:
        ...
        warc_files = session.query(WARCFile).filter_by(snapshot_id=snapshot_id).all()
        ...
        for warc_file in warc_files:
            ...
            success = indexer.generate_and_store_index(warc_file.id, snapshot_id)
...
Examples:
  ...
  python generate_cdx_indexes.py --batch --limit 100
    ...
    parser.add_argument(
        "--batch", "-b", action="store_true", help="Batch index all unindexed WARCs"
    )

    parser.add_argument(
        "--warc-id", "-w", type=int, help="Index specific WARC file by ID"
    )

    parser.add_argument(
        "--snapshot-id", "-s", type=int, help="Index all WARCs for a snapshot"
    )
    ...
    if args.batch:
        ...
        success_rate = (
            stats["successful"] / stats["total_found"] * 100
